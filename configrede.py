...
configrede = configparser.ConfigParser()

confPadrao = 'rede.ini'
if (os.path.exists(confPadrao)):
    configrede.read(confPadrao)
else:
...
def runParser():
    ...
    parser.add_argument('-i', '--inicial', action='store', dest='cpfcnpjInicial', default=configrede['INICIO'].get('cpfcnpj', ''), type=str, help='1 ou mais cnpj separados por ponto e vírgula; Nome ou Razao Social Completa')
    parser.add_argument('-c', '--camada', action='store', dest='camadaInicial', type=int, default=configrede['INICIO'].getint('camada', 1), help='camada')
    ...
    parser.add_argument('-m', '--n-mensagem', action='store_false', dest='bExibeMensagemInicial', default=configrede['INICIO'].getboolean('exibe_mensagem_advertencia', True), help='não exibe mensagem inicial')
    parser.add_argument('-M',  '--mensagem', action='store_true', dest='bExibeMensagemInicial', default=configrede['INICIO'].getboolean('exibe_mensagem_advertencia', True), help='exibe mensagem inicial')
    parser.add_argument('-y','--n-menuinserir', action='store_false', dest='bMenuInserirInicial', default=configrede['INICIO'].getboolean('exibe_menu_inserir', True), help='não exibe menu para inserir no inicio')
    parser.add_argument('-Y','--menuinserir', action='store_true', dest='bMenuInserirInicial', default=configrede['INICIO'].getboolean('exibe_menu_inserir', True), help='exibe menu para inserir no inicio')
    parser.add_argument('-d','--download', action='store_true', dest='bArquivosDownload', default=configrede['ETC'].getboolean('arquivos_download', False), help='permitir download da pasta arquivos')
    parser.add_argument('-D','--n-download', action='store_false', dest='bArquivosDownload', default=configrede['ETC'].getboolean('arquivos_download', False), help='permitir download da pasta arquivos')
...
referenciaBD = configrede['BASE'].get('referencia_bd', '')
if referenciaBD:
